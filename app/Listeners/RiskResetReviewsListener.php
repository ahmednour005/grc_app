...
class RiskResetReviewsListener
{
     ...
     * Handle the event.
     *
     * @param  \App\Events\RiskResetReviews  $event
     * @return void
     */
    public function handle(RiskResetReviews $event)
    {
         // Get the action ID for Risk_Add
         $action1 = Action::where('name', 'Risk_Reset_Reviews')->first();
         $actionId1 = $action1['id'];
         
         // Get the risk object from the event
         $risk = $event->risk;
         $resetRiskReviews = $event->resetRiskReviews;
         ...
         $risk->additional_stakeholder_id = $additionalStakeholdersNames;
         $risk->owner_id = $risk->owner ? $risk->owner->name : null;
         $risk->team_id = $teamsNames;
         // $risk->owner_id = $risk->owner ? $risk->owner->name : null;
