...
class RiskCloseListener
{
     ...
     * Handle the event.
     *
     * @param  \App\Events\RiskClose  $event
     * @return void
     */
    public function handle(RiskClose $event)
    {
        // Get the action ID for Risk_Add
        $action1 = Action::where('name', 'Risk_Close')->first();
        ...
        $risk->source_id = $risk->source ? $risk->source->name : null;
        ...
        $risk->Note=$event->note;
        // $risk->close_reason =$event->close_reason->closereasons->name;
