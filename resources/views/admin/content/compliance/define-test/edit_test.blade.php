<div class="modal modal-slide-in basic-select2 fade bootstrap-select" id="edit-test">
  <div class="modal-dialog sidebar-sm">
    <form action="" method="PUT" class="modal-content pt-0" id="edit-record">
      @csrf
      <input name="test_id" type="hidden" value="">
      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">×</button>
      <div class="modal-header mb-1">
        <h5 class="modal-title" id="exampleModalLabel">{{ __('compliance.edit-test') }}</h5>
      </div>
      <div class="modal-body flex-grow-1">
        <div class="mb-1">
          <label class="form-label " for="select2-basic1">{{ __('compliance.ControlName') }}</label>
          <select class="select2 form-select" id="edit-framework_control_id" name="framework_control_id" >
        ...
        <div class="mb-1">
          ...
          <select name="additional_stakeholders[]" class="form-select multiple-select2"  multiple="multiple">
            <option value="">{{ __('locale.select-option') }}</option>
             @foreach($testers as $tester)
              <option value="{{$tester->id}}">{{$tester->name}}</option>
             @endforeach
